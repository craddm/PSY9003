...
What's missing is *c* - the *total* effect of the IV on the DV.
]
...
openplotmat(asp = 1)
textrect(c(.1, .5), .1, lab = "Room temp", radx = .2)
textrect(c(.9, .5), .1, lab = "Amount drunk", radx = .2)
straightarrow(c(.2, .5), c(.8, .5))
```
]
...
```{r highlight.output = 12}
summary(lm(affect ~ estress, data = estress))
```
...
`estress` wasn't a significant a predictor of `withdraw`, but was a significant predictor of `affect`.

`affect` was a signficant predictor of `withdraw`.

Modelling `withdraw` as a function of `estress` and `affect` changed the coefficient of `estress`.

The *indirect* effect of `estress` is the the coefficient of `estress` from Step 1 - the coefficent of `estress` from Step 3.
...
```{r}
med_model <- med(data = estress, dep = "withdraw", pred = "estress", med = "affect", paths = TRUE, estPlot = TRUE)
med_model$med
```
